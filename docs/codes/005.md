---
description: The error `SHA-pinned action requires a version comment`. Use when a uses line pinned to a bare SHA is rejected, to see why the fork network makes such a SHA unverifiable, and how to resolve it by dropping `--no-api` or ignoring the action with `rules`.
---
