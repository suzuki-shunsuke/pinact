...
When combined with `--fix`, pinact v3.10.0 or newer automatically corrects the version comment to match the actual SHA:

```console
$ pinact run --verify --fix testdata/bar.yaml
...
Use `--check` instead of `--fix` to preview changes without modifying the file.
