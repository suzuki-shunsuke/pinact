---
description: Verify that a version comment names the version its SHA actually is (`--verify-comment`). Use when pinact reports a version annotation mismatch, when it rewrites a comment to another version, or when reviewing a workflow for a forged version comment.
---
...
pinact v4.0.0 or newer automatically corrects the version comment to match the actual SHA:

```console
$ pinact run --verify testdata/bar.yaml
WARN[0000] version annotation mismatch detected, correcting comment program=pinact version="" workflow_file=testdata/bar.yaml line_number=15 line="      - uses: actions/checkout@ee0669bd1cc54295c223e0bb666b733df41de1c5 # v3.5.1" action=actions/checkout@ee0669bd1cc54295c223e0bb666b733df41de1c5 action=actions/checkout action_version=ee0669bd1cc54295c223e0bb666b733df41de1c5 version_annotation=v3.5.1 commit_hash_of_version_annotation=83b7061638ee4956cf7545a6f7efe594e5ad0247 correct_version=v2.7.0 help_docs=https://github.com/suzuki-shunsuke/pinact/blob/main/docs/codes/001.md
```

The version comment is corrected from `v3.5.1` to `v2.7.0` to match the actual SHA.
Use `--fix=false` or `--check` to preview changes without modifying the file.
